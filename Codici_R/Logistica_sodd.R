...
id=which(dati$Soddisfazione_Laurea==5&dati$Soddisfazione_Preparazione==5)
sodmax<-rep(0,n)
sodmax[id]<-1
...
#Occhio che però sarà un po' scontato, magari da lì vedere se trovi altre cose interessanti
...
best<-step(compl)
summary(best)
#qui però non c'è più niente di significativo, meglio le altre due (che tra l'altro facendo l'anova trovo essere equivalenti al modello completo)
...
for ( k in 1 : lens )
{
  ...
  ascissa_roc [ k ] = t.misc [ 2, 1 ] /( t.misc [ 2, 1 ] + t.misc [ 2, 2 ] )
     ...
     ordinata_roc[k]=ifelse(t.misc[1]==1,1,0)
       ascissa_roc[k]=ifelse(t.misc[1]==1,1,0)
   }
...
#Secondo me il problema è che la probabilità non va mai sopra il 60% e poi il campione si ferma di punto in bianco a 111 quindi la curva roc viene troncata
